  ...
  # Establecer nombre de filas utilizando una columna(columna "Country")
  # Servirá para poner estos nombres en el dendograma
...
#-------------------------------------------------------------------------------
#----CUTTREE---- VIEW CLUSTER INSIDE
    
    k = 8   # Clústers
    
    fit <- cutree(hc2,  # árbol
                  k=k)  # número de clústers (muestra a que clúster pertenece cada uno de los registros)
    
    table(fit)  # muestra cuantos elementos tiene cada clúster
    
    #Marca los clústers (genera rectángulos con borde del color indicado)
    par(mfrow=c(1,1))
...
##----- CLUSTERING DIVISITIVO----------------------------------------------------------
  ...
  # Plot
  par(mfrow=c(1,2))   # Graficar los dos plots (1 fila y dos columnas)
  plot(dv)  #(Al hacer ENTER en consola, mostrará diferentes gráficos)
